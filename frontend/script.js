...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchPersonsMusicPieceId(personId) {
    try {
      ...
      const personInfo = data.famous_people.find(
  ...
  async function fetchMusicPieceInfo(personId) {
    ...
    try {
      const response = await fetch(`../backend/music_pieces.json`);
  ...
  function handleImageClick(event) {
    ...
    fetchMusicPieceInfo(personId)
      .then((musicPieceInfo) => {
        ...
        window.location.href = `piece_page.html?title=${encodeURIComponent(
          musicPieceInfo.title
        )}&composer=${encodeURIComponent(
          musicPieceInfo.composer
        )}&id=${encodeURIComponent(musicPieceInfo.spotify_id)}`;
      })
      .catch((error) => {
